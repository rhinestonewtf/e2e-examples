...
export function useGlobalWallet() {
  ...
  // Get the connected wallet address from any connected wallet
  ...
  }, [ready, authenticated, wallets, address, isConnected]);
      ...
      try {
        ...
        const formattedPortfolio: TokenBalance[] = portfolio
          ...
          .filter((token: any) => {
            ...
            console.log(
              `Filtering ${token.symbol}: hasBalance=${token.hasBalance}`
            );
  ...
  }, [ready, isConnected, address, walletClient]);
