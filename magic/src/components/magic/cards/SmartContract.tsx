    ...
    if (web3 && contractAddress) {
      ...
      try {
        ...
        if (BigInt(accountBalance) < transactionCost) {
          throw new Error(
            `Insufficient funds for gas * price + value: have ${accountBalance}, need ${transactionCost}`
          );
        }
  ...
  return (
    <Card>
      ...
      <CardLabel
        ...
        rightAction={
          !contractAddress ? <Spinner /> : <div onClick={copy}>{copied}</div>
        }
      />
      ...
      <div>
        <div>
          ...
          {value !== undefined ? value.toString() : "...loading"}
        </div>
        <Spacer size={20} />
        <FormInput
          type="number"
